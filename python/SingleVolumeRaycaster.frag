...
void rayTraversal(in vec3 first, in vec3 last, float entryDepth, float exitDepth) {
	...
	//vec3 r = rayDirection * tIncr * rand(gl_FragCoord.xy);
	//first += r;
    ...
    WHILE(!finished) {
		...
		vec3 g = CALC_GRADIENT(volume_, volumeStruct_, samplePos);
		vec3 pos = samplePos + length (g)*tIncr*rand(gl_FragCoord.xy)*rayDirection;
		// read RGBA volume
		color = textureLookup3D(volume_, volumeStruct_, pos);
		color = textureLookup3D(volume_, volumeStruct_, samplePos);
