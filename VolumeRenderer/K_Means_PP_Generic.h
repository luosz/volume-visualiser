	...
	public:
		...
		*/
		...
		static void k_means(const std::vector<T> & data, const int k, unsigned char *& label_ptr, real get_distance(const T & v1, const T & v2), T get_centroid(const std::vector<T> & list))
		{
			...
			char *centroids_filename = "d:/centroids.txt";
			ofstream print_centroids(centroids_filename, ios::out);
			print_centroids<<k<<endl;
			for (int i=0; i<k; i++)
			{
				print_centroids<<centroids->at(i).x<<" "<<centroids->at(i).y<<" "<<centroids->at(i).z<<endl;
			}
			print_centroids.close();
			std::cout<<"Centroids are saved to "<<centroids_filename<<endl;
#endif
